...
function Showprogress4() {
    var indicator = document.querySelector('div.indicator');
    var preview4 = document.querySelector('div.preview4');
    var divand = document.createElement('div');
    divand.setAttribute('id', 'progressand');
    divand.style.display = "none";
    indicator.appendChild(divand);
...
function previewFile4() {
    console.log("Function called");
    var preview4 = document.querySelector('div.preview4');
    var files = document.querySelector('input[name=fileAva]').files;
    console.log(files.length);
    function readAndPreview4(file) {
        if (/\.(jpe?g|png|gif|mp4|webm|ogg|ogv|mp3|wav)$/i.test(file.name)) {
            if (file.size < 10 * 1024 * 1024) { // MAX 10 mb
                var reader = new FileReader();
                var spc = '\u00A0';
    ...
    if (files) {
        [].forEach.call(files, readAndPreview4);
    }
}

function previewFile2() {
    console.log("Function called previewFile2");
    var preview2 = document.querySelector('div.preview2');
...
function previewFiles2() {
    var preview2 = document.querySelector('div.preview2');
    var files = document.querySelector('input[name=files2]').files;
    ...
    if (files) {
        [].forEach.call(files, readAndPreview2);
    }
}
...
function Showprogress3() {
    var indicator = document.querySelector('div.indicator');
    var preview3 = document.querySelector('div.preview3');
    var divand = document.createElement('div');
    divand.setAttribute('id', 'progressand');
    divand.style.display = "none";
    indicator.appendChild(divand);
...
function previewFile3() {
    ...
    if (files) {
        [].forEach.call(files, readAndPreview3);
    }
}
                                            ...
                                            var clickLink = function (event) {
                                                ...
                                                emojiInput.value = emojiInput.value.substring(0, caretPos) + " " + event.target.innerHTML + emojiInput.value.substring(caretPos), emojiPicker.style.display = "block", "undefined" != typeof angular && angular.element(emojiInput).triggerHandler("change")
                                                    ...
                                                    clickCategory = function (event) {
                                                        ...
                                                        for (var hideUls = emojiPicker.querySelectorAll("ul"), i = 1, l = hideUls.length; i < l; i++)
                                                            hideUls[i].style.display = "none";
                                                        var backgroundToggle = emojiPicker.querySelectorAll(".category a");
                                                        for (i = 0, l = backgroundToggle.length; i < l; i++)
