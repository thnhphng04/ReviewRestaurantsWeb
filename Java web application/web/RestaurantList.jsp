    ...
    <div class="wrapper">
                    ...
                    <div class="box box-primary">
                        <div class="box-body">
                            <table width="100%" class="table table-hover table-striped" id="dataTables-example">
                                ...
                                <tbody>
                                    <c:forEach items="${requestScope.resList}" var="c">
                                        <tr>
                                            <td>${c.restaurantId}</td>
                                            <td>${c.accountId}</td>
                                            <td>${c.name}</td>
                                            <td style="color: ${c.status == 1 ? 'blue' : 
                                                                c.status == 2 ? 'orange' : 
                                                                c.status == 3 ? 'red' : 
                                                                c.status == 4 ? 'purple' : 
                                                                c.status == 6 ? 'red' : 'black'}">
                                                    ${c.status == 1 ? 'Hoạt động' : 
                                                      c.status == 2 ? 'Đang chờ duyệt' : 
                                                      c.status == 3 ? 'Ẩn bởi admin' : 
                                                      c.status == 4 ? 'Bài chỉnh sửa' : 
                                                      c.status == 6 ? 'Báo cáo' : 'Người dùng ẩn'}
                                                </td>
                                                <td><a class="v" href="RestaurantPage?restaurantId=${c.restaurantId}">Xem </a></td>
                                                <td class="text-end">
                                                    <c:if test="${c.status==1}">
                                                        <div class="button-container">
                                                            <a href="ChangeRestaurantStatus?id=${c.restaurantId}&status=${c.status}">
                                                                <button type="button" data-button="accept" onclick="accept()">Đổi trạng thái nhà hàng</button>
                                                            </a>
        ...
        <div id="deny-popup" style="display: none;">
            <div id="deny-popup-dialog">
                <h2 style="">Lý do từ chối</h2>
                <p style="">Xaccs nhận rằng bạn từ chối nhà hàng với lý do sau</p>
                <form action="AcceptPostRequest" method="POST">
                    <textarea id="deny-reason" name="deny-reason" placeholder="Nhập lý do của bạn"></textarea>
                    <input type="hidden" id="post-id" name="post-id" value="" />
                    <button type="button" data-button="cancel" onclick="hideDenyPopup()" style="color: red">Cancel</button>
                    <button type="submit" data-button="submit" style="color: green">Submit</button>
                </form>
            </div>
        </div>>
