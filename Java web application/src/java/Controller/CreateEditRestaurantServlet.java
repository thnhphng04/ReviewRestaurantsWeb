 ...
 */
@MultipartConfig
@WebServlet(name="CreateEditRestaurantServlet", urlPatterns={"/createEditRestaurant"})
public class CreateEditRestaurantServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CreateEditRestaurantServlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CreateEditRestaurantServlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        String editOf = request.getParameter("editOfRestaurantId");
        ...
        rd.hideRestaurantForEditting(editOf);
        rd.insertEditRestaurant(name, address, phoneNumber, lowestPrice, highestPrice, openTime, closeTime, accountId, filePathAva, editOf);
        
        String message = "Bạn đã tạo bản chỉnh sửa thành công!";

        response.sendRedirect("RestaurantPage?restaurantId="+ editOf +"&message=" + URLEncoder.encode(message, "UTF-8"));
