...
public class VerifyEmailServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (authCode.isEmpty()) {
            ...
            response.sendRedirect("VerifyEmail.jsp?key_code=" + email);
            return;
        }
        AccountDetail acc = accDAO.getAccountByEmail(email);
        String verifyCode = String.valueOf(acc.getVerifyCode());
       
        if (authCode.equals(verifyCode)) {
            accDAO.setStatus(email);
            
            String message = "Bạn đã tạo tài khoản thành công!";
            
            response.sendRedirect("login?message=" + URLEncoder.encode(message, "UTF-8"));
        } else {
            request.setAttribute("err", "Invalid verification code");
            response.sendRedirect("VerifyEmail.jsp?key_code=" + email);
        }
