...
<head>
    ...
    <script>
        ...
        function showDenyPopup(postId, reportId) {
...
<!--                <li>
                    ...
                    <div class="box box-primary">
                        <div class="box-body">
                            <table width="100%" class="table table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    <c:forEach items="${requestScope.listPostReport}" var="c">
                                        <tr>
                                            <td>${c.postReportId}</td>
                                            <td>${c.postId}</td> 
                                            <td>${c.accountId}</td>
                                            <c:choose>
                                                <c:when test="${c.typeOfReport == 1}">
                                                    <td>Nội dung không liên quan đến địa điểm</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 2}">
                                                    <td>Hình ảnh đồi trụy</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 3}">
                                                    <td>Bài viết PR/Quảng cáo</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 4}">
                                                    <td>Nội dung sai lệch/ Tin giả</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 5}">
                                                    <td>Từ ngữ bài post không phù hợp</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 6}">
                                                    <td>Bài viết rác/ Spam</td>
                                                </c:when>
                                                <c:when test="${c.typeOfReport == 7}">
                                                    <td>Khác</td>
                                                </c:when>
                                            </c:choose>
                                            <td>${c.reason}</td>
                                            ...
                                            <td class="text-end">
                                                <div class="button-container">
                                                    <button type="button" data-button="accept" onclick="showDenyPopup('${c.postId}', '${c.postReportId}')">Chấp nhận</button>
                                                    
                                                <a href="AcceptPostReport?postId=${c.postId}&reportId=${c.postReportId}" >
    ...
    <div id="deny-popup" style="display: none;">
        <div id="deny-popup-dialog">
            ...
            <form action="AcceptPostReport" method="POST">
